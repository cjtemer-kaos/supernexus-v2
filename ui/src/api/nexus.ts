...
/**
 * Drop-in replacement for fetch() that auto-injects auth.
 * Use: `import { authFetch } from "@/api/nexus"` then replace `fetch(` with `authFetch(`
 */
...
export async function sendChatMessage(message: string, gem = "auto", images: string[] = []): Promise<ChatResponse> {
