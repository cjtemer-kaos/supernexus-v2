...
def _is_safe_path(target: Path) -> bool:
    """Allow any local path except critical system directories."""
    try:
        resolved = str(target.resolve()).replace("\\", "/").lower()
        # Block only critical system paths
        _BLOCKED = ["/windows/system32", "/windows/syswow64", "/program files/windowsapps"]
        return not any(b in resolved for b in _BLOCKED)
    except Exception:
        return False
