...
if AGENT == "all":
    targets = ["opencode", "claude-code", "antigravity", "openclaw", "qwen-code", "hermes-code", "zero-code", "aider-code"]
else:
    targets = [AGENT]
...
for t in targets:
    ...
    if count > 0:
        ...
        if t == "qwen-code":
            script = os.path.join(os.path.dirname(__file__), "..", "agents", "qwen_autonomous_loop.py")
        elif t == "hermes-code":
            script = os.path.join(os.path.dirname(__file__), "..", "agents", "hermes_autonomous_loop.py")
        elif t == "zero-code":
            script = os.path.join(os.path.dirname(__file__), "..", "agents", "zero_autonomous_loop.py")
        elif t == "aider-code":
            script = os.path.join(os.path.dirname(__file__), "..", "agents", "aider_autonomous_loop.py")
        else:
            script = os.path.join(os.path.dirname(__file__), "..", "agents", "nexus_autonomous_loop.py")
        run_hidden([sys.executable, script, t])
    else:
