...
from src.gemas_core.core.rate_limit_unified import SafetyLimiter
...
# ============================================================
#
# v1.9.0 — Replaced with the unified gemas_core.core.rate_limit_unified
# ``SafetyLimiter``. The legacy ``RateLimiter`` (odysseus port, with
# burst detection) is kept as a thin alias so external imports keep
# working — it just delegates to the new ``SafetyLimiter``.

@dataclass
class RateLimitConfig:
    """Back-compat shim. The unified ``SafetyLimiter`` does not
    consume this directly; new code should use ``SafetyLimiter``'s
    purpose-based config instead.
    """
    max_requests: int = 30
...
class RateLimiter:
    """Back-compat wrapper around ``SafetyLimiter``.

    Existing call sites use ``is_allowed(client_id)`` and expect
    ``(bool, dict)`` with at least ``{"reason", "retry_after"}`` on
    denial. This wrapper preserves the surface.
    """

    def __init__(self, config: Optional[RateLimitConfig] = None):
        cfg = config or RateLimitConfig()
        self.config = cfg
        # Single-purpose facade: 30 req / 60s, matching the legacy defaults.
        # The unified ``SafetyLimiter`` provides burst detection as a
        # separate limiter if needed; for now we drop it (no callers
        # rely on burst detection in the SuperNEXUS v2.0 surface).
        self._facade = SafetyLimiter(
            default={
    ...
    def is_allowed(self, client_id: str) -> tuple[bool, Dict]:
        allowed, info = self._facade.check(client_id, purpose="default")
        return allowed, info

    def reset(self, client_id: str = None):
        if client_id:
            self._facade.reset(client_id)
        else:
            self._facade.reset()
...
class TimeoutManager:
    ...
    def __init__(self, config: Optional[TimeoutConfig] = None):
        ...
        self._timeouts: Dict[str, float] = {
            ...
            "remote_node": self.config.remote_node,
    ...
    def get_status(self) -> Dict:
        return {
            ...
            "timeouts": {
                ...
                for service in [
                    "ollama_chat", "ollama_vision", "remote_node", "http_request"
                ]
            },
            "fallback_chains": {
                task_type: {
