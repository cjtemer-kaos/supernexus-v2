...
class SetupWizard:
    ...
    DEFAULT_CONFIG = {
        ...
        "pc2": {
            ...
            "port": 22,
            "user": "",
            "password_env": "SUPER_NEXUS_PC2_PASSWORD",
        },
        "tailscale": {
            "enabled": False,
    ...
    def run(self) -> Dict:
        ...
        # PC2
        print("\n2. PC2 (ejecucion remota - opcional)")
        enable_pc2 = input("   Habilitar PC2? (y/N): ").strip().lower()
        if enable_pc2 == "y":
            config["pc2"]["enabled"] = True
            pc2_host = input(f"   Host [{config['pc2']['host']}]: ").strip()
            if pc2_host:
                config["pc2"]["host"] = pc2_host
            config["pc2"]["user"] = input("   Usuario SSH: ").strip()
            print("   Password se lee de variable de entorno: NEXUS_PC2_PASSWORD")
        else:
            config["pc2"]["enabled"] = False
    ...
    def validate(self) -> Dict:
        ...
        # Verificar PC2
        if config.get("pc2", {}).get("enabled"):
            if not config["pc2"].get("user"):
                issues.append("PC2 habilitado pero no hay usuario SSH")
            import os
            if not os.environ.get("NEXUS_PC2_PASSWORD"):
                issues.append("PC2 habilitado pero NEXUS_PC2_PASSWORD no definida")
