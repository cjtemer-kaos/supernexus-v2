...
class NEXUSGuardrails:
    ...
    Combina input validation, output validation, tool access control, y LLM Gate Classifier.
    """
    ...
    def __init__(
        ...
        self._llm_gate = None
        if llm_executor:
            from security.llm_gate_classifier import LLMGateClassifier
            self._llm_gate = LLMGateClassifier(
    ...
    def check_tool_access(self, tool: str, action: str, confirmed: bool = False) -> Dict:
        """Verifica acceso a herramienta (basado en reglas)"""
    ...
    async def classify_tool_call(
        ...
        """
        Clasifica llamada a herramienta usando LLM Gate (allow/confirm/block).
        Si no hay LLM configurado, cae a reglas.
        """
        if not self._llm_gate:
            result = self.check_tool_access(tool_name, str(tool_input)[:100])
            if result.get("allowed"):
                return {"verdict": "allow", "reason": "Rule-based allow", "blocked": False}
            if result.get("requires_confirmation"):
                return {"verdict": "confirm", "reason": result.get("reason", ""), "blocked": False}
            return {"verdict": "block", "reason": result.get("reason", ""), "blocked": True}

        from security.llm_gate_classifier import GateVerdict
        decision = await self._llm_gate.classify(tool_name, tool_input, context)
        return {
            "verdict": decision.verdict.value,
            "reason": decision.reason,
            "is_irreversible_delete": decision.is_irreversible_delete,
            "confidence": decision.confidence,
            "blocked": decision.verdict == GateVerdict.BLOCK,
        }
    ...
    def get_security_report(self) -> Dict:
        ...
        return {
            ...
            "llm_gate_enabled": self._llm_gate is not None,
