...
class InputValidator:
    ...
    def sanitize_path(cls, path: str) -> str:
        """Sanitiza ruta de archivo — usa Path.resolve() para evitar traversal."""
...
class AuditLogger:
    ...
    def get_stats(self) -> Dict:
        ...
        total = len(self.logs)
        successes = len([entry for entry in self.logs if entry.status == "success"])
        failures = len([entry for entry in self.logs if entry.status == "failure"])
        
        unique_users = len(set(entry.user for entry in self.logs))
        unique_ips = len(set(entry.ip for entry in self.logs))
