...
class LocalEmbedder:
    ...
    Singleton: el modelo SentenceTransformer se carga una sola vez,
    aunque se creen multiples instancias de LocalEmbedder.
    """
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        ...
        if LocalEmbedder._shared_model is not None and LocalEmbedder._shared_model_name == model_name:
            self.model = LocalEmbedder._shared_model
            self._available = LocalEmbedder._shared_available
        else:
            self.model = None
            self._available = False
            self._try_load()
            if self._available:
                LocalEmbedder._shared_model = self.model
                LocalEmbedder._shared_available = True
                LocalEmbedder._shared_model_name = model_name
