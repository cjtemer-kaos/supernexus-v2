        ...
        if url:
            for pattern in PRIVATE_IP_PATTERNS:
                if re.search(pattern, url, re.IGNORECASE):
                    return HookResult(allow=False, message="Permission denied: SSRF protection — URL targets private network")
            ...
            if url:
                for pattern in PRIVATE_IP_PATTERNS:
                    if re.search(pattern, url, re.IGNORECASE):
                        return MiddlewareResult(
                            action=MiddlewareAction.STOP,
                            message="Permission denied: SSRF protection — URL targets private network",
                        )
