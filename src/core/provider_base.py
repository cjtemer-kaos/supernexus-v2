        ...
        **kwargs: Any,
        ...
        for attempt in range(max_retries):
            ...
            if resp.finish_reason != "error":
                return resp
            # Patron openhuman: clasificar el error y abortar si es non-retryable
            ...
            non_retryable_hints = (
                ...
                "permission denied", "model not found", "model unknown",
                "context window exceeded", "context length exceeded",
            )
            is_non_retryable = any(h in content_lower for h in non_retryable_hints)
            if is_non_retryable:
                logger.warning(
                    "Provider %s non-retryable error: %s",
                    self.name, (resp.content or "")[:200],
                )
                return resp
            # Detectar Retry-After header (openhuman parse_retry_after_ms)
...
class FallbackProvider(LLMProvider):
    ...
    async def chat(self, messages: list[LLMMessage], tools: list[dict] | None = None, **kwargs: Any) -> LLMResponse:
        ...
        if not self._tripped and self._primary:
            ...
            content_blank = resp.content is None or not resp.content.strip()
            if resp.finish_reason != "error" and not content_blank:
                self._consecutive_failures = 0
                return resp

            is_fallback_kind = resp.content and any(k in resp.content.lower() for k in self.FALLBACK_KINDS)
            if is_fallback_kind or content_blank:
...
def _resolve_api_key(profile: ProviderProfile) -> str:
    """Resuelve api_key: primero de api_key_env, luego de api_key directa."""
    import os
...
def create_provider_from_profile(profile: ProviderProfile) -> LLMProvider:
    ...
    # Map provider_type aliases
    pt = profile.provider_type
    if pt in ("zen",):
        pt = "openai"
    ...
    if pt in ("openai", "openrouter", "groq"):
