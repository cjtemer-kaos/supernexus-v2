#!/usr/bin/env python3
"""
Engram Skill - Memoria Persistente Avanzada para Nexus IA
Implementa el protocolo de memoria definido en engram/SKILL.md
"""
...
# DB Path (Alineado con nexus.py)
DB_PATH = Path("memory/nexus_memory.db")
...
class EngramSkill:
    ...
    def mem_save(self, content: str, category: str = "observation", project: str = "nexus", metadata: dict = None):
        ...
        with self._get_conn() as conn:
            ...
            conn.commit()
        return f"[OK] Engram guardado en {category}: {content[:50]}..."
