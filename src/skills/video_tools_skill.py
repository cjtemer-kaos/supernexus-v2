...
class VideoToolsSkill:
    def __init__(self):
        ...
        self.tools = {
            "open-codesign": {
                ...
                "path": os.environ.get("OPEN_CODESIGN_PATH", str(Path(__file__).resolve().parents[2] / "external" / "open-codesign")),
                "cmd": "pnpm dev"
            },
            "claude-code": {
                "name": "Claude-Code",
            ...
            "locally-uncensored": {
                ...
                "path": os.environ.get("BOLT_DIY_PATH", str(Path(__file__).resolve().parents[2] / "external" / "bolt_diy")),
    ...
    def launch(self, tool_id: str):
        ...
        if tool_id in self.tools:
            ...
            if not os.path.isdir(cwd):
                return f"[Error] Ruta no configurada para {tool_id}. Setea la variable de entorno correspondiente o instala en: {cwd}"
            try:
    ...
    def launch_dual_terminal(self):
        ...
        cwd = os.environ.get("OPEN_CODESIGN_PATH", str(Path(__file__).resolve().parents[2] / "external" / "open-codesign"))
        if not os.path.isdir(cwd):
            return f"[Error] Open CoDesign no instalado. Configura OPEN_CODESIGN_PATH o instala en: {cwd}"
        try:
