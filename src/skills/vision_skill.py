class VisionSkill:
    ...
    def analyze(self, image_path=None, custom_prompt=None, model="qwen2.5vl:7b"):
        ...
        # Configuración centralizada: Remote Node (Hermes) es el motor de visión soberano
        Remote Node_IP = "100.83.38.20"
        OLLAMA_URL = f"http://{Remote Node_IP}:11434"
        ...
        try:
            ...
            if r.status_code == 200:
                return {"response": r.json().get("response", ""), "image": image_path, "node": "Remote Node", "model": VISION_MODEL}
            else:
                return {"error": f"Ollama Remote Node Error: {r.text}", "status": r.status_code}
        except Exception as e:
            return {"error": f"Fallo de conexión con Remote Node Vision: {str(e)}"}
...
class ImageAnalyzer:
    ...
    def process_image(self, image_path, context=""):
        ...
        Remote Node_IP = "100.83.38.20"
        OLLAMA_URL = f"http://{Remote Node_IP}:11434/api/generate"
