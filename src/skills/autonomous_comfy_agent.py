...
# Configuración
PC2_OLLAMA_URL = "http://100.83.38.20:11434/api/generate"
PC2_MINIMAX_URL = "http://100.83.38.20:8000/v1/chat/completions" # Asumiendo API de Minimax en PC2
...
def get_vision_analysis(image_path):
    ...
    try:
        res = requests.post(PC2_OLLAMA_URL, json=payload, timeout=30)
        return res.json().get("response", "")
    except Exception as e:
        return f"Error de visión: {e}"

def ask_minimax_for_action(vision_data, goal):
    # Aquí es donde el cerebro de PC2 toma la decisión pesada
    # Usamos un prompt de razonamiento avanzado
    prompt = f"""
    CONTEXTO VISUAL: {vision_data}
    OBJETIVO: {goal}
    
    Eres el cerebro de automatización de Nexus corriendo en PC2.
    Tu tarea es decidir el siguiente movimiento de ratón o teclado en CJTR.
...
def main():
    ...
    print(f"--- AGENTE AUTÓNOMO NEXUS (Cerebro: Hermes | Cuerpo: CJTR) ---")
