...
class SkillSync:
    """
    Sincronizacion de skills entre Windows y PC2.
    ...
    Uso:
        ...
        port = int(os.getenv('NEXUS_PORT', '9000'))
        await sync.auto_sync(remote_url=f"http://{os.getenv('SUPER_NEXUS_PC2_IP', 'localhost')}:{port}")
    """
    ...
    async def auto_sync(self, remote_url: str = None) -> Dict:
        ...
        if not remote_url:
            from src.core.nexus_config import get_nexus_url, get_port
            pc2_ip = os.getenv("SUPER_NEXUS_PC2_IP", "localhost")
            if self.node_id == "local":
                remote_url = f"http://{pc2_ip}:{get_port()}"
            else:
                remote_url = get_nexus_url()
