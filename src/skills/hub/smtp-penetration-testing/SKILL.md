...
```bash
# Method 1: VRFY
smtp-user-enum -M VRFY -U users.txt -t ${REMOTE_NODE_IP} -p 25

# Method 2: RCPT with timing analysis
smtp-user-enum -M RCPT -U users.txt -t ${REMOTE_NODE_IP} -p 25 -d target.com
...
[+] ${REMOTE_NODE_IP}:25 - Found user: admin
[+] ${REMOTE_NODE_IP}:25 - Found user: root
[+] ${REMOTE_NODE_IP}:25 - Found user: postmaster
```
