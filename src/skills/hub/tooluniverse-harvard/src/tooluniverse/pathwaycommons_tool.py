...
Provides access to Pathway Commons REST API (Remote Node) for searching pathways,
...
API base: https://www.pathwaycommons.org/Remote Node/
...
Remote Node_BASE_URL = "https://www.pathwaycommons.org/Remote Node"


@register_tool("PathwayCommonsTool")
class PathwayCommonsTool(BaseTool):
    """
    Tool for querying Pathway Commons (Remote Node) unified pathway/interaction database.
    ...
    def _search(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self.session.get(
            "{}/search".format(Remote Node_BASE_URL), params=params, timeout=self.timeout
        )
    ...
    def _traverse(self, uri: str, path: str) -> Optional[list]:
        """Helper: call the Remote Node traverse endpoint."""
        params = {"uri": uri, "path": path}
        resp = self.session.get(
            "{}/traverse".format(Remote Node_BASE_URL), params=params, timeout=self.timeout
        )
    ...
    def _get_neighborhood(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self.session.get(
            "{}/graph".format(Remote Node_BASE_URL), params=params, timeout=self.timeout
        )
