...
net ${REMOTE_NODE_IP}/24

# Exclude specific traffic
not arp

# Combine filters
host ${REMOTE_NODE_IP} and port 443
```
...
```
# IP address filters
ip.addr == ${REMOTE_NODE_IP}              # All traffic to/from IP
ip.src == ${REMOTE_NODE_IP}               # Source IP only
ip.dst == ${REMOTE_NODE_IP}               # Destination IP only
...
```
# AND operator
ip.addr == ${REMOTE_NODE_IP} && tcp.port == 80
...
(ip.src == ${REMOTE_NODE_IP} || ip.src == ${REMOTE_NODE_IP}) && tcp.port == 443
```
