...
Ou crie um arquivo `.env` em `${USER_HOME}\skills\ai-studio-image\`:
```
...
```bash
pip install -r ${USER_HOME}\skills\ai-studio-image\scripts\requirements.txt
```

## 3. Gerar Sua Primeira Imagem

```bash
python ${USER_HOME}\skills\ai-studio-image\scripts\generate.py --prompt "mulher jovem tomando cafe em cafeteria" --mode influencer --format square
```
...
```bash
python ${USER_HOME}\skills\ai-studio-image\scripts\prompt_engine.py --prompt "prompt do usuario" --mode influencer
```
...
```bash
python ${USER_HOME}\skills\ai-studio-image\scripts\generate.py \
  --prompt "prompt humanizado gerado no passo anterior" \
  --mode influencer \
  --format square \
  --model gemini-2-flash-exp \
  --output ${USER_HOME}\skills\ai-studio-image\data\outputs\
```
...
```bash
python ${USER_HOME}\skills\ai-studio-image\scripts\templates.py --list
```
