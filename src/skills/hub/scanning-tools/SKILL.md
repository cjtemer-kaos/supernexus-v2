...
```bash
# Host discovery
nmap -sn ${REMOTE_NODE_IP}/24              # Ping scan (no port scan)
nmap -sL ${REMOTE_NODE_IP}/24              # List scan (DNS resolution)
nmap -Pn ${REMOTE_NODE_IP}               # Skip host discovery

# Port scanning techniques
nmap -sS ${REMOTE_NODE_IP}               # TCP SYN scan (stealth)
nmap -sT ${REMOTE_NODE_IP}               # TCP connect scan
nmap -sU ${REMOTE_NODE_IP}               # UDP scan
nmap -sA ${REMOTE_NODE_IP}               # ACK scan (firewall detection)

# Port specification
nmap -p 80,443 ${REMOTE_NODE_IP}         # Specific ports
nmap -p- ${REMOTE_NODE_IP}               # All 65535 ports
nmap -p 1-1000 ${REMOTE_NODE_IP}         # Port range
nmap --top-ports 100 ${REMOTE_NODE_IP}   # Top 100 common ports

# Service and OS detection
nmap -sV ${REMOTE_NODE_IP}               # Service version detection
nmap -O ${REMOTE_NODE_IP}                # OS detection
nmap -A ${REMOTE_NODE_IP}                # Aggressive (OS, version, scripts)

# Timing and performance
nmap -T0 ${REMOTE_NODE_IP}               # Paranoid (slowest, IDS evasion)
nmap -T4 ${REMOTE_NODE_IP}               # Aggressive (faster)
nmap -T5 ${REMOTE_NODE_IP}               # Insane (fastest)

# NSE Scripts
nmap --script=vuln ${REMOTE_NODE_IP}     # Vulnerability scripts
nmap --script=http-enum ${REMOTE_NODE_IP}  # Web enumeration
nmap --script=smb-vuln* ${REMOTE_NODE_IP}  # SMB vulnerabilities
nmap --script=default ${REMOTE_NODE_IP}  # Default script set

# Output formats
nmap -oN scan.txt ${REMOTE_NODE_IP}      # Normal output
nmap -oX scan.xml ${REMOTE_NODE_IP}      # XML output
nmap -oG scan.gnmap ${REMOTE_NODE_IP}    # Grepable output
nmap -oA scan ${REMOTE_NODE_IP}          # All formats
```
...
```bash
# Basic scanning
masscan -p80 ${REMOTE_NODE_IP}/24 --rate=1000
masscan -p80,443,8080 ${REMOTE_NODE_IP}/24 --rate=10000

# Full port range
masscan -p0-65535 ${REMOTE_NODE_IP}/24 --rate=5000

# Large-scale scanning
masscan 0.0.0.0/0 -p443 --rate=100000 --excludefile exclude.txt

# Output formats
masscan -p80 ${REMOTE_NODE_IP}/24 -oG results.gnmap
masscan -p80 ${REMOTE_NODE_IP}/24 -oJ results.json
masscan -p80 ${REMOTE_NODE_IP}/24 -oX results.xml

# Banner grabbing
masscan -p80 ${REMOTE_NODE_IP}/24 --banners
```
...
nessuscli scan --create --name "Internal Scan" --targets ${REMOTE_NODE_IP}/24
nessuscli scan --list
