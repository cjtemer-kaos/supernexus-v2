...
async def test_Remote Node_bridge(r: TestResult):
    """Test Remote Node bridge"""
    from src.bridges.Remote Node_bridge import Remote NodeBridge

    try:
        Remote Node = Remote NodeBridge()
        status = Remote Node.get_status()

        if status["host"] == "${REMOTE_NODE_IP}":
            r.ok("Remote Node bridge config")
        else:
            r.fail("Remote Node bridge config", f"Host: {status['host']}")

        # Don't actually connect (may not be available)
        r.ok("Remote Node bridge initialized")
    except Exception as e:
        r.fail("Remote Node bridge", str(e))
...
async def main():
    ...
    tests = [
        ...
        ("Ollama", test_ollama),
        ("Remote Node Bridge", test_Remote Node_bridge),
